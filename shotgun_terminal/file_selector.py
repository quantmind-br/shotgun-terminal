...
from .tree_selector import run_hierarchical_selector, run_simple_selector
...
class FileSelector:
    ...
    def interactive_selection(self):
        ...
        console.print("\n[bold blue]File Selection Method[/bold blue]")

        selection_method = inquirer.list_input(
            "Choose file selection method",
            choices=[
                (
                    "Hierarchical Tree (Recommended) - Visual tree with checkboxes",
                    "hierarchical",
                ),
                (
                    "Traditional Pattern-based - Configure ignore patterns",
                    "traditional",
                ),
            ],
        )
        ...
        if selection_method == "hierarchical":
            return self._hierarchical_selection()
        else:
            return self._traditional_selection()

    def _hierarchical_selection(self):
        """Use the hierarchical tree selector."""
        console.print("[blue]Opening Rich hierarchical file selector...[/blue]")
        console.print("[dim]Navigate with commands: up/down, space, enter, s, q[/dim]")
        console.print("[dim]Type 'help' for detailed instructions[/dim]\n")

        try:
            selected_files = run_hierarchical_selector(self.directory)

            if selected_files:
                # Convert Path objects to relative strings for compatibility
                relative_files = [
                    str(f.relative_to(self.directory)) for f in selected_files
                ]
                console.print(
                    f"\n[green]✓[/green] Selected {len(relative_files)} files using hierarchical selector"
                )
                return (
                    relative_files,
                    [],
                )  # Ignore patterns are managed by .shotgunignore
            else:
                console.print(
                    "[yellow]No files selected or operation cancelled[/yellow]"
                )
                console.print("Falling back to traditional selection...")
                return self._traditional_selection()

        except Exception as e:
            console.print(f"[red]Error in hierarchical selector:[/red] {e}")
            console.print("Falling back to traditional selection...")
            return self._traditional_selection()

    def _traditional_selection(self):
        """Traditional pattern-based file selection."""
        # Get all files
        ...
        while True:
            ...
            if action == "View selected files":
                ...
                if pattern:
                    ...
                    selected_files = [
                        f for f in selected_files if not fnmatch.fnmatch(f, pattern)
                    ]
                ...
                if len(selected_files) > 50:
                    console.print(
                        "[yellow]Too many files for individual selection. Use pattern removal instead.[/yellow]"
                    )
                else:
                    questions = [
                        inquirer.Checkbox(
                ...
                if excluded_files:
                    if len(excluded_files) > 50:
                        console.print(
                            "[yellow]Too many excluded files. Use pattern matching to add back.[/yellow]"
                        )
                        pattern = Prompt.ask("Enter pattern to add back (e.g., '*.py')")
                        if pattern:
                            added_files = [
                                f for f in excluded_files if fnmatch.fnmatch(f, pattern)
                            ]
                            selected_files.extend(added_files)
                            console.print(
                                f"[green]Added back {len(added_files)} files[/green]"
                            )
                    else:
                        questions = [
                            inquirer.Checkbox(
                        ...
                        console.print(
                            f"[green]Added back {len(answers['files'])} files[/green]"
                        )
                else:
