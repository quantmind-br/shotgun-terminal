...
class GeminiService:
    ...
    def configure(self, api_key: str) -> bool:
        ...
        if not GEMINI_AVAILABLE:
            console.print(
                "[red]Error:[/red] google-genai package not available. Install with: pip install google-genai"
            )
            return False
        ...
        try:
            ...
            console.print("[red]Error configuring Gemini API:[/red]", str(e))
            self.is_configured = False
            return False
    ...
    def generate_response(
        self, prompt: str, temperature: float = 0.35, thinking_budget: int = 32768
    ) -> Optional[str]:
        ...
        if not (0.0 <= temperature <= 2.0):
            console.print(
                "[red]Error:[/red] Temperature must be between 0.0 and 2.0, got",
                temperature,
            )
            return None
        ...
        if not (0 <= thinking_budget <= 32768):
            console.print(
                "[red]Error:[/red] Thinking budget must be between 0 and 32768, got",
                thinking_budget,
            )
            return None
        ...
        try:
            ...
            with Progress(
                ...
                task = progress.add_task(
                    "Receiving response from Gemini...", total=None
                )
                ...
                for chunk in self.client.models.generate_content_stream(
                    ...
                    if chunk.text:
                        ...
                        progress.update(task, description=f"Receiving: {safe_preview}")
    ...
    def save_response(self, response: str, output_dir: str = ".") -> Optional[str]:
        ...
        try:
            ...
            console.print("[green]✓[/green] Response saved to:", str(filepath))
            return str(filepath)
