...
class SettingsManager:
    ...
    def get_gemini_settings(self) -> Dict[str, Any]:
        """Get Gemini settings."""
        settings = self.load_settings()
        return settings.get("geminiSettings", {
            "api_key": "",
            "enable_gemini": False,
            "temperature": 0.35,
            "thinking_budget": 32768
    ...
    def set_gemini_settings(self, api_key: str, enable_gemini: bool = True, 
                           temperature: float = 0.35, thinking_budget: int = 32768):
        """Set Gemini settings."""
        # Validate parameters
        ...
        if not (0 <= thinking_budget <= 32768):
            raise ValueError(f"Thinking budget must be between 0 and 32768, got {thinking_budget}")
