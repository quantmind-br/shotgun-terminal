...
class TranslationService:
    ...
    def _initialize_client(self):
        ...
        if api_key and base_url:
            try:
                ...
                console.print(f"[dim]Translation API initialized with base URL: {base_url}[/dim]")
            except Exception as e:
                console.print(f"[yellow]Warning: Failed to initialize API client: {e}[/yellow]")
                self.client = None
        else:
            console.print(f"[dim]Translation API not configured (api_key: {'✓' if api_key else '✗'}, base_url: {'✓' if base_url else '✗'})[/dim]")
            self.client = None
    ...
    def translate_to_english(self, text: str, text_type: str = "text", force: bool = False) -> Optional[str]:
        """
        ...
        if not force:
            ...
            console.print(f"[dim]Language detection for {text_type}: {'English' if is_english else 'Portuguese'}[/dim]")
            
            if is_english:
                console.print(f"[blue]Text appears to be in English already, skipping translation[/blue]")
                return text
        else:
            console.print(f"[yellow]Forcing translation for {text_type} (language detection bypassed)[/yellow]")
...
Return only the translated text without any additional commentary."""
    ...
    def _is_likely_english(self, text: str) -> bool:
        ...
        strong_portuguese_indicators = [
            'ção', 'ções', 'ão', 'ões', 'nh', 'lh', 'ç'
        ]
        
        # Common Portuguese words
        portuguese_words = [
            'que', 'para', 'com', 'uma', 'não', 'são', 'está', 'foi', 'ser', 'ter',
            'fazer', 'implementar', 'criar', 'adicionar', 'corrigir', 'quando', 'onde',
            'função', 'método', 'classe', 'arquivo', 'código', 'aplicação', 'sistema',
            'usuário', 'dados', 'página', 'botão', 'erro', 'problema'
        ]
        
        text_lower = text.lower()
        
        # Check for strong Portuguese patterns first
        strong_count = sum(1 for indicator in strong_portuguese_indicators if indicator in text_lower)
        if strong_count > 0:
            return False  # Definitely Portuguese
        
        # Check for Portuguese words
        word_count = sum(1 for word in portuguese_words if f' {word} ' in f' {text_lower} ' or text_lower.startswith(f'{word} ') or text_lower.endswith(f' {word}'))
