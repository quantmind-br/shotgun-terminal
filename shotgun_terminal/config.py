...
class ConfigManager:
    ...
    def configure(self):
        ...
        console.print(
            Panel.fit(
                "[bold cyan]🔧 Shotgun Configuration[/bold cyan]\n"
        ...
        while True:
            ...
            if action == "Translation API (OpenAI-compatible)":
                self._configure_api()
            elif action == "Gemini API":
    ...
    def _configure_api(self):
        ...
        console.print(
            Panel.fit(
                "[bold blue]🔧 Translation API Configuration[/bold blue]\n"
    ...
    def _configure_credentials(self):
        ...
        new_key = Prompt.ask(
            "Enter API Key", default=current_key if current_key else ""
        )
        ...
        new_url = Prompt.ask(
            "Enter Base URL",
            default=current_url if current_url else "https://copilot.quantmind.com.br",
        )
        ...
        if new_key and new_url:
            self.settings.set_api_settings(
                new_key, new_url, new_model, enable_translation
            )
    ...
    def _test_connection(self):
        ...
        if not self.translator.is_configured():
            console.print(
                "[red]Error:[/red] API not configured. Please configure credentials first."
            )
            return
    ...
    def _show_current_settings(self):
        ...
        masked_key = (
            f"{api_key[:8]}...{api_key[-4:]}" if len(api_key) > 12 else "Not configured"
        )
        ...
        table.add_row(
            "Translation Enabled",
            "Yes" if settings.get("enable_translation", True) else "No",
        )
        table.add_row(
            "Status",
            "Configured" if self.translator.is_configured() else "Not configured",
        )
    ...
    def _reset_to_defaults(self):
        ...
        if Confirm.ask(
            "Reset all API settings to defaults? This will remove your credentials.",
            default=False,
        ):
    ...
    def quick_setup(self):
        ...
        if self.translator.test_connection():
            ...
            console.print(
                "[red]✗[/red] Connection test failed. Please check credentials."
            )
    ...
    def _configure_gemini(self):
        ...
        console.print(
            Panel.fit(
                "[bold yellow]🤖 Gemini API Configuration[/bold yellow]\n"
    ...
    def _configure_gemini_credentials(self):
        ...
        new_key = Prompt.ask(
            "Enter Gemini API Key", default=current_key if current_key else ""
        )
    ...
    def _test_gemini_connection(self):
        ...
        if not api_key:
            console.print(
                "[red]Error:[/red] Gemini API not configured. Please configure credentials first."
            )
            return
    ...
    def _configure_gemini_parameters(self):
        ...
        console.print(
            f"[blue]Current Temperature:[/blue] {current_temp} (range: 0.0 - 2.0)"
        )

        while True:
            try:
                new_temp_str = Prompt.ask(
                    "Enter Temperature", default=str(current_temp)
                )
                ...
                if 0.0 <= new_temp <= 2.0:
                    ...
                    console.print(
                        "[red]Error:[/red] Temperature must be between 0.0 and 2.0"
                    )
            except ValueError:
        ...
        console.print(
            f"[blue]Current Thinking Budget:[/blue] {current_budget} (range: 0 - 32768)"
        )

        while True:
            try:
                new_budget_str = Prompt.ask(
                    "Enter Thinking Budget", default=str(current_budget)
                )
                ...
                if 0 <= new_budget <= 32768:
                    ...
                    console.print(
                        "[red]Error:[/red] Thinking budget must be between 0 and 32768"
                    )
            except ValueError:
    ...
    def _toggle_gemini(self):
        ...
        if new_enabled:
            ...
            if not api_key:
                console.print(
                    "[yellow]Warning:[/yellow] Gemini is enabled but no API key is configured"
                )
    ...
    def _show_current_gemini_settings(self):
        ...
        table.add_row("API Key", masked_key)
        table.add_row(
            "Gemini Enabled", "Yes" if settings.get("enable_gemini", False) else "No"
        )
    ...
    def _reset_gemini_to_defaults(self):
        ...
        if Confirm.ask(
            "Reset all Gemini settings to defaults? This will remove your API key.",
            default=False,
        ):
            default_gemini_settings = self.settings._get_default_settings()[
                "geminiSettings"
            ]
            self.settings.set_gemini_settings(
