...
class TreeGenerator:
    ...
    def generate_tree(
        self, included_files: List[str], ignore_patterns: List[str] = None
    ) -> str:
        """
    ...
    def _build_tree_lines(
        self, structure: dict, lines: list, prefix: str, is_root: bool = False
    ):
        ...
        for i, item in enumerate(items):
            ...
            if is_root:
                current_prefix = (
                    self.tree_chars["last"] if is_last else self.tree_chars["branch"]
                )
            else:
            ...
            if structure[item]:
                ...
                if is_root:
                    child_prefix = (
                        self.tree_chars["space"]
                        if is_last
                        else self.tree_chars["extension"]
                    )
                else:
                    child_prefix = prefix + (
                        self.tree_chars["space"]
    ...
    def _traverse_directory(
        ...
        try:
            ...
            for i, item in enumerate(items):
                ...
                if is_root:
                    current_prefix = (
                        self.tree_chars["last"]
                        if is_last
                        else self.tree_chars["branch"]
                    )
                else:
                    current_prefix = prefix + (
                        self.tree_chars["last"]
                ...
                if item.is_dir() and depth < max_depth:
                    ...
                    self._traverse_directory(
                        item, lines, child_prefix, depth + 1, max_depth
                    )
